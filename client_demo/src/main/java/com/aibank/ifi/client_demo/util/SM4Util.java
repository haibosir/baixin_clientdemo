...
public class SM4Util {
	...
	/**
	 * 分组加密，内容长度不足时补充字符（控制符DEL的对应十进制为127，解密时移除）
	 * 
	 * @param plaintext
	 * @param key
	 * @return
	...
	public static byte[] encrypt(byte[] plaintext, byte[] key) {
		...
		int size = length % 16 == 0 ? length : ((length / 16 + 1) * 16);
		if (size >= length) {
			byte[] plainNew = new byte[size];
			System.arraycopy(plaintext, 0, plainNew, 0, length);
			for (int i = length; i < size; i++) {
				plainNew[i] = 127;
			}
			return encodeSMS4(plainNew, key);
		}
		return null;
	}
	...
	public static byte[] decrypt(byte[] ciphertext, byte[] key) {
		...
		int length = plaintext.length;
		int newlen = 0;
		for (int i = length - 1; i > length - 1 - 16; i--) {
			if (plaintext[i] != 127) {
				newlen = i + 1;
				break;
			}
		}
		byte[] newplain = new byte[newlen];
		System.arraycopy(plaintext, 0, newplain, 0, newlen);
		return newplain;
	}
